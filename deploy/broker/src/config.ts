...
import { parseMoney } from "@olurabian/purse";
...
export interface Config {
  ...
  adminToken: string; executor: ExecutorConfig; maxPending: number; otel: boolean; openPolicy: boolean;
}
...
const validateMoney = (name: string, value: string): void => {
  try { parseMoney(value); } catch { throw new ConfigError(`${name} is not a money amount ("${value}"); use a form like "$50"`); }
};
...
export function loadConfig(env: Env = process.env, readFile: (path: string) => string = (p) => readFileSync(p, "utf8")): Config {
  ...
  if (env.PURSE_MAX_PER_ACTION) { validateMoney("PURSE_MAX_PER_ACTION", env.PURSE_MAX_PER_ACTION); policy.maxPerAction = env.PURSE_MAX_PER_ACTION; }
  if (env.PURSE_MAX_PER_DAY) { validateMoney("PURSE_MAX_PER_DAY", env.PURSE_MAX_PER_DAY); policy.maxPerDay = env.PURSE_MAX_PER_DAY; }
  if (env.PURSE_REQUIRE_APPROVAL_OVER) { validateMoney("PURSE_REQUIRE_APPROVAL_OVER", env.PURSE_REQUIRE_APPROVAL_OVER); policy.requireApprovalOver = env.PURSE_REQUIRE_APPROVAL_OVER; }
  ...
  const hasPolicy = Boolean(policy.allow?.length) || Boolean(policy.maxPerAction);
  if (!hasPolicy && env.PURSE_ALLOW_OPEN_POLICY !== "1") {
    throw new ConfigError("no policy: set PURSE_ALLOW or PURSE_MAX_PER_ACTION, or set PURSE_ALLOW_OPEN_POLICY=1 to run with no limits on purpose");
  }
  const openPolicy = !hasPolicy && env.PURSE_ALLOW_OPEN_POLICY === "1";
  ...
  if (kind === "x402") {
    ...
    if (signer === "evm") {
      ...
      if (env.PURSE_X402_KEY_FILE) {
        try { raw = readFile(env.PURSE_X402_KEY_FILE).trim(); }
        catch (e) { throw new ConfigError(`cannot read PURSE_X402_KEY_FILE: ${(e as Error).message}`); }
      } else {
        raw = env.PURSE_X402_PRIVATE_KEY?.trim();
      }
  ...
  return { policy, store, ports, adminToken, executor, maxPending: int("PURSE_MAX_PENDING", env.PURSE_MAX_PENDING, 100), otel: Boolean(env.OTEL_EXPORTER_OTLP_ENDPOINT), openPolicy };
}
